...
function GoPrevPage() {
  ...
  if (pageNow == lastPage - 2 || pageNow == lastPage - 1) {
    ...
    $(".pagination__number")[pageNow - lastPage + 5].classList.remove("pagination__number--selected");
    $(".pagination__number")[pageNow - lastPage + 4].classList.add("pagination__number--selected");
  } else if (pageNow > 2 && pageNow < lastPage - 1) {
    $(".pagination__next--skip").css("display", "block");
    var pageNumbers = $(".pagination__number");
    for (var i = 1; i < pageNumbers.length - 1; i++) {
      pageNumbers[i].innerHTML = pageNow - 2 + i;
    }
    if (pageNow == 3) {
      $(".pagination__prev--skip").css("display", "none");
    }
  } else if (pageNow == 1 || pageNow == 2) {
    $(".pagination__number")[pageNow].classList.remove("pagination__number--selected");
    $(".pagination__number")[pageNow-1].classList.add("pagination__number--selected");
    if (pageNow == 1) {
...
function GoNextPage() {
  ...
  if (pageNow == 2 || pageNow == 3) {
    ...
    $(".pagination__number")[pageNow-2].classList.remove("pagination__number--selected");
    $(".pagination__number")[pageNow-1].classList.add("pagination__number--selected");
  } else if (pageNow > 3 && pageNow < lastPage - 1) {
    $(".pagination__prev--skip").css("display", "block");
    var pageNumbers = $(".pagination__number");
    for (var i = 1; i < pageNumbers.length - 1; i++) {
      pageNumbers[i].innerHTML = pageNow - 2 + i;
    }
    if (pageNow == lastPage - 2) {
      $(".pagination__next--skip").css("display", "none");
    }
  } else if (pageNow == lastPage - 1 || pageNow == lastPage) {
    $(".pagination__number")[pageNow - lastPage + 3].classList.remove("pagination__number--selected");
    $(".pagination__number")[pageNow - lastPage + 4].classList.add("pagination__number--selected");
    if (pageNow == lastPage) {
